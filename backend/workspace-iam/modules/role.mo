...
module RoleModule {
	...
	public class RoleService(_storage : RoleCollection, policyService : PolicyModule.PolicyService) {
		public func getAll() : [Role] {
			let roleIter = Map.vals<Text, Role>(_storage);
		...
		public func getById(rid : Text) : ?Role {
			return Map.get<Text, Role>(_storage, thash, rid);
		};
		...
		public func create(data : { displayName : Text; policies : [Text] }) : async Role {
			...
			if (maybeExistingRole == null) {
				...
				ignore Map.put(_storage, thash, rid, newRole);
				...
				case (?role, ?_policy) {
					...
					switch (policyAlreadyGranted) {
						...
						case null {
							...
							ignore Map.put(_storage, thash, rid, updatedRole);
						};
					};
				};
				case (?_role, null) {
