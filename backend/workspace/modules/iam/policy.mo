...
module PolicyModule {
	...
	public class PolicyService(_repository : PolicyCollection) {
		public func getAll() : [Policy] {
			let policyIter = Map.vals<Text, Policy>(_repository);
		...
		public func getById(pid : Text) : ?Policy {
			return Map.get<Text, Policy>(_repository, thash, pid);
		};
		...
		public func create(data : Policy) : async () {
			...
			ignore Map.put<Text, Policy>(_repository, thash, data.pid, data);
		};
		...
		public func delete(pid : Text) : async Policy {
			switch (getById(pid)) {
				case (?policy) {
					...
					ignore Map.remove<Text, Policy>(_repository, thash, pid);
