...
const formSchema = z.object({
  roles: z.array(z.string()).min(1, "At least one permission is required"),
});
...
const ModalAssignRoleUser: FC<ModalProps> = ({ showModal, setShowModal, dataNameSearch,assignPrincipal,assignRoles }) => {
  ...
  const getRoles = async () => {
		...
		if ("ok" in getRolesResult) {
			const filteredRoles = getRolesResult.ok.filter((role) => {
				return !assignRoles.includes(role.action);
			});
  ...
  const getSelectedRolesText = () => {
    if (selectedRoleValues.length === 0) return "Select Permissions";
	...
	const onSubmit: SubmitHandler<FormValues> = async (data) => {
		...
		try {
			for (const role of data.roles) {
				const response = await workspaceUser.add_permission_to_access(Principal.fromText(assignPrincipal), role);
				if ("err" in response) {
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-6 rounded shadow-lg w-1/3">
        <form className="flex flex-col gap-y-6" onSubmit={handleSubmit(onSubmit)}>
          <h2 className="text-xl mb-4">Assign Permissions</h2>
